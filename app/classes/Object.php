...
class Objects
{
	...
	public function storeCustomerOrderInvoice($invoice_number, $customer_name, $orderdate, $find_customer_name, $total_quantity, $orderQuantity, $price, $totalPrice, $pro_name, $pid, $subtotal, $discount, $prev_due, $netTotal, $paidBill, $dueBill, $payMethode)
	{
		...
		// First check if we have enough quantity before creating invoice
		for ($i = 0; $i < count($price); $i++) {
			if ($total_quantity[$i] < $orderQuantity[$i]) {
				return "Sorry ! you haven't the quantity for product: " . $pro_name[$i];
			}
		}
		...
		if ($invoice_no != null) {
			for ($i = 0; $i < count($price); $i++) {
				...
				if ($remain_quantity < 0) {
					...
					$stmt->bindParam(1, $remain_quantity, PDO::PARAM_INT);
					$stmt->bindParam(2, $pid[$i], PDO::PARAM_INT);
					$stmt->execute();
				}
			...
			$payment_query = array(
				...
				'invoice_number' => $invoice_number, // Add invoice_number to link payment to specific invoice
