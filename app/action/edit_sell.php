...
if (isset($_POST)) {
	...
	// Get the invoice number to delete the related payments
	$invoice_info = $obj->find('invoice', 'id', $invoice_id);
	$invoice_number = $invoice_info->invoice_number;

	// Delete all payments related to this specific invoice
	$payment_del_query = $pdo->prepare("DELETE FROM `sell_payment` WHERE `invoice_number` = ?");
	$payment_del_query->bindParam(1, $invoice_number, PDO::PARAM_STR);
	...
	// Get the invoice number from the existing invoice
	$invoice_info = $obj->find('invoice', 'id', $invoice_id);
	$invoice_number = $invoice_info->invoice_number;
