...
if (isset($_POST)) {
	...
	if (!empty($payment_date) && !empty($pay_amount)) {
		...
		// This payment is not tied to a specific invoice - it's a general payment for the customer account
		$pay_inser_query['invoice_number'] = 'GENERAL-' . time(); // Generate a general reference that clearly indicates it's not tied to a regular invoice
